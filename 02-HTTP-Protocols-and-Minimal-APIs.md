# 2. โพรโทคอล HTTP และการพัฒนา Minimal APIs สำหรับงาน IoT
...
## 2.1 สภาพแวดล้อมและเครื่องมือพัฒนา .NET CLI

ก่อนเริ่มพัฒนา Minimal API สำหรับ IoT Gateway ควรตรวจสอบสภาพแวดล้อมของเครื่องพัฒนา

### 2.1.1 การตรวจสอบเวอร์ชัน .NET SDK
เปิด Terminal หรือ PowerShell แล้วรันคำสั่ง
```powershell
dotnet --version
```
การตรวจสอบรายการ SDK ทั้งหมดที่ติดตั้งอยู่ในเครื่อง
```powershell
dotnet --list-sdks
```

### 2.1.2 คำสั่ง CLI สำคัญสำหรับวงจรการพัฒนา
* **`dotnet new web -o <ProjectName>`**
  สร้างโครงสร้างโปรเจกต์เว็บเปล่าแบบ Minimal
* **`dotnet build`**
  คอมไพล์โปรเจกต์เพื่อตรวจสอบไวยากรณ์และข้อผิดพลาด
* **`dotnet run`**
  คอมไพล์และเปิดรัน Kestrel Web Server
* **`dotnet watch`**
  รันเว็บเซิร์ฟเวอร์ในโหมด **Hot Reload** (เมื่อแก้ไขและบันทึกโค้ด ระบบจะอัปเดตการทำงานอัตโนมัติทันทีโดยไม่ต้องกด Stop/Run ใหม่)

---

## 2.2 วงจรการทำงานของโพรโทคอล HTTP (HTTP Lifecycle)
...
1. **Headers** 
   ข้อมูลเมทาดาทา (Metadata) เช่น `Content-Type: application/json`, `Content-Length: 48`
2. **Body (Payload)** 
   ข้อมูลจริง เช่น สตริง JSON หรือไบนารีข้อมูล (มีเฉพาะในคำขอแบบ POST/PUT หรือในการตอบกลับ)

---
## 2.3 วิวัฒนาการสู่ Minimal APIs ใน Modern C# 
...
### 2.3.1 Minimal APIs  เขียนโค้ดที่กระชับ ตรงไปตรงมา
...
## 2.4 การเปิดรับการเชื่อมต่อข้ามเครื่อง (LAN Binding & CORS)
...
## 2.5 กลไก JSON Serialization อัตโนมัติ (Zero-Config JSON)
...
## 2.6 การออกแบบ Data Contract สำหรับข้อมูล Telemetry
...
## 2.7 การรับคำสั่งควบคุม (Control Endpoints --  Route & Query Parameters)
...
### 2.7.1 Route Parameter (รับค่าจาก URL Path)
```csharp
...
### 2.7.2 Query Parameter (รับค่าพารามิเตอร์ต่อท้ายเครื่องหมาย ?)
```csharp
