# ใบงานการทดลองที่ 8.1 (Labsheet 8.1): พื้นฐาน Kestrel Web Server และ Minimal API
...
### 🌟 กิจกรรมที่ 1: ตรวจสอบความพร้อมของระบบและสร้างโปรเจกต์ใน 30 วินาที 

1. เปิดโปรแกรม **Terminal** หรือ **PowerShell** ใน VS Code แล้วพิมพ์คำสั่งตรวจสอบเวอร์ชัน:
   ```bash
   dotnet --version
   ```
   > 📌 *ผลลัพธ์ควรแสดงเป็นตัวเลข เช่น `8.0.xxx` หรือ `9.0.xxx`*

2. สร้างโฟลเดอร์สำหรับการทดลองและสร้างโปรเจกต์ Web ว่างเปล่า (Empty Web):
   ```bash
   # สร้างโฟลเดอร์และเข้าไปด้านใน
   mkdir IoT_Gateway && cd IoT_Gateway

   # สร้างโปรเจกต์ Web ว่างเปล่า
   dotnet new web -o .
   ```

3. ทดลองสั่งรันเว็บเซิร์ฟเวอร์ทันที:
   ```bash
   dotnet run
   ```
...
### 🌟 กิจกรรมที่ 2: ผ่าดูโครงสร้างและเขียนโค้ดสั้นที่สุดในโลก
