# บทเรียนที่ 1: สถาปัตยกรรม Edge Web Server และ Kestrel Engine
...
> 3. สามารถเทียบเคียงแนวคิดระหว่าง POSIX C Socket Programming กับ Modern C# Web Framework
> 4. ตระหนักถึงความสำคัญของการจัดการหน่วยความจำแบบ Non-blocking I/O ในงานอุปกรณ์ IoT
> 5. เข้าใจการวิเคราะห์ปัญหาเชิงประจักษ์ในสภาวะโหลดสูง (The C10K Problem, Context Switching, และ Non-blocking I/O)

---

## 1. วิวัฒนาการของ Web Server ในงาน IoT

### 1.1 ทำไมต้องก้าวข้าม LAMP Stack?
...
1. **การแยกส่วนของโปรเซส (Process Fragmentation):** ต้องติดตั้ง Apache/Nginx แยกกับตัวรันภาษา (PHP/Python) และต้องคอนฟิก Web Server ให้ชี้ไปยังโฟลเดอร์ไฟล์สคริปต์
2. **การกินทรัพยากรสูง (High Resource Footprint):** โมเดล Fork Process หรือสร้าง Thread ใหม่ต่อทุกๆ HTTP Request กิน RAM สูง ไม่เหมาะกับอุปกรณ์ Embedded
3. **ขาดการเข้าถึง Hardware ระดับลึก:** สคริปต์เว็บส่วนใหญ่รันอยู่บน Sandboxed Environment ทำให้การเปิดพอร์ต Serial (UART), ดักฟังขา GPIO, หรือสื่อสารกับไมโครคอนโทรลเลอร์ทำได้ยากและล่าช้า

#### ทางออกยุคใหม่: Self-Hosted Single-Binary Gateway
ในสถาปัตยกรรม IoT ยุคใหม่ เราต้องการเซิร์ฟเวอร์ที่เป็น **"Single Executable"** (ไฟล์ไบนารีเดียว) ที่มี Web Server คุณภาพสูงฝังอยู่ในตัวกระบวนการ (In-Process Web Server) ซึ่งสามารถรันได้ทันทีโดยไม่ต้องติดตั้ง Web Server ภายนอกใดๆ เพิ่มเติม
...
### 1.2 การวิเคราะห์ปัญหาเชิงประจักษ์ในสภาวะโหลดสูง (Empirical Analysis & Proof of Bottlenecks)
...
## 2. รู้จัก Kestrel Web Server — หัวใจของ Modern .NET
...
### 2.1 Logical View (มุมมองเชิงตรรกะ)
...
## 3. สถาปัตยกรรม Non-blocking I/O และ `System.IO.Pipelines`
...
- **Zero-Allocation Parsing**: Kestrel จะไม่แปลงไบต์ของ Header ให้เป็น String ในหน่วยความจำหากไม่จำเป็น แต่จะใช้ Pointer/Span ชี้ไปยังบัฟเฟอร์เดิม ช่วยลดภาระของ Garbage Collector (GC) อย่างมหาศาล
- **Thread Pool Integration**: การรอข้อมูลจากเครือข่ายจะไม่บล็อกเธรด (Non-blocking I/O) ทำให้เซิร์ฟเวอร์ขนาดเล็กบน Edge สามารถรองรับการเชื่อมต่อพร้อมกันได้หลายร้อยไคลเอนต์

---

## 4. ตารางเปรียบเทียบแนวคิด: C Programming vs Modern C# (Kestrel)

เนื่องจากนักศึกษาสาย IoT มักมีความคุ้นเคยกับการเขียนภาษา C บนไมโครคอนโทรลเลอร์ ตารางนี้จะช่วยเทียบเคียงแนวคิดให้เห็นภาพชัดเจน:

| มิติการทำงาน | ภาษา C (POSIX / WinSock Socket) | Modern C# (Kestrel Web Server) |
| :--- | :--- | :--- |
| **Build File & Dependencies** | `Makefile` หรือคำสั่ง `gcc -lws2_32` | ไฟล์ `.csproj` (`<Project Sdk="Microsoft.NET.Sdk.Web">`) |
| **Header Inclusions** | `#include <sys/socket.h>`, `#include <stdio.h>` | `using Microsoft.AspNetCore.Builder;` (หรือ Implicit Usings) |
| **จุดเริ่มต้นโปรแกรม (Entry)** | `int main(int argc, char* argv[])` | `Top-level statements` (โค้ดรันได้ทันทีโดยไม่ต้องครอบคลาส) |
| **การสร้างและผูก Socket** | `socket()`, `bind()`, `listen()` | `builder.WebHost.ConfigureKestrel(opts => opts.ListenAnyIP(5000))` |
| **Event Loop รับการเชื่อมต่อ** | วนลูป `accept()` คู่กับ `select()` หรือ `epoll()` | จัดการอัตโนมัติด้วย Kestrel Socket Engine + ThreadPool |
| **Buffer รับส่งข้อมูล** | `char buffer[1024]; recv(); send();` | จัดการผ่าน `HttpContext` (`context.Request`, `context.Response`) |
| **Callback จัดการ Request** | ฟังก์ชันพอยน์เตอร์ `void (*handler)(int sock)` | Request Delegate เช่น `app.MapGet("/", () => "Hello")` |
