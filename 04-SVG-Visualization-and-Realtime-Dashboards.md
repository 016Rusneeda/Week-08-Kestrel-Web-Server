# บทเรียนที่ 4: การแสดงผลเชิงภาพด้วย SVG และแดชบอร์ด IoT แบบตอบสนองทันที
...
## 1. ทำไมต้อง SVG สำหรับ Industrial IoT Dashboard?

ในงานพัฒนาหน้าจอแสดงผลของระบบควบคุมและ IoT วิศวกรมักเผชิญทางเลือกระหว่าง:
1. **การใช้ Canvas API (`<canvas>`)**: เป็น Raster Bitmap วาดยาก ขยายหน้าจอแล้วภาพเบลอ ตกแต่งด้วย CSS ไม่ได้
2. **การโหลดไลบรารีภายนอก (Chart.js / D3.js / ECharts)**: ขนาดไฟล์ใหญ่ (หลักร้อย KB ถึง MB), ต้องพึ่งพาอินเทอร์เน็ตในการดึง CDN, และกินทรัพยากรเบราว์เซอร์สูง
3. **Scalable Vector Graphics (SVG)**: **ทางเลือกที่ดีที่สุดสำหรับ Edge Dashboard!**
...
## 2. คณิตศาสตร์มาตรวัด: การแปลงสเกลเชิงเส้น (Linear Mapping)

สัญญาณจากไมโครคอนโทรลเลอร์ ESP32 มักเป็นค่า **ADC 12-bit ($0 - 4095$)** แต่หน้าปัดมาตรวัด (Speedometer) ต้องการค่าเป็น **มุมองศา ($\theta$)**:
...
## 3. เทคนิคเส้นโค้งความยาว Arc ด้วย `stroke-dasharray` และ `stroke-dashoffset`
...
## 4. เทคนิค CSS Transition เพื่อความนุ่มนวล (Buttery Smooth Movement)
...
## 5. สถาปัตยกรรม Client-Side Polling
...
## 6. แค็ตตาล็อก SVG วิดเจ็ตมาตรฐาน (Widget Showcase)

### 6.1 Speedometer Gauge (มาตรวัดความเร็วทรงกลม)
- **ส่วนประกอบ**:
  - `<circle>` หรือ `<path>` ทำเป็นรางสเกลโค้ง (Arc)
  - `<line>` หรือ `<polygon>` ทำเป็นเข็มไมล์ชี้ค่า
  - `<circle>` ตรงกลางทำเป็นหมุดครอบแกนหมุน

### 6.2 VU Meter (LED Bar Graph สไตล์เสียง/อุตสาหกรรม)
- **ส่วนประกอบ**:
...
### 6.3 Retro 7-Segment Display (หน้าจอตัวเลข 7 ส่วน)
- **ส่วนประกอบ**:
